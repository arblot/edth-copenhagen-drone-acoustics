...
- EMA weights, AMP, waveform time-shift (train only)
- Eval-time TTA (separate for val/test), label smoothing, (optional) balanced sampler
- Early stopping, seeded loaders, gradient clipping
"""
...
class FusionDataset(Dataset):
    def __init__(self, filepaths: List[Path], labels: List[int], cfg: SpecConfig,
                 ...
                 n_mels_override: int | None = None):
        self.filepaths = filepaths
        self.labels = labels
        self.cfg = cfg if (n_mels_override is None) else dc_replace(cfg, n_mels=int(n_mels_override))
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path("fusion_model.pt"),
                        help="Where to save final best model (EMA).")
    ...
    # Probe tab dim
    ds_probe = FusionDataset([X_tr[0]], [int(y_tr[0])], cfg=cfg, time_crop=args.time_crop, n_mels_override=args.n_mels)
    ...
    train_ds_warm = FusionDataset(list(X_tr), list(map(int, y_tr)), cfg=cfg, time_crop=args.time_crop,
                                  ...
                                  sa_p=args.sa_p,
                                  enable_shift=True, shift_prob=args.shift_prob, shift_max_sec=args.shift_max_sec,
    ...
    val_ds   = FusionDataset(list(X_val), list(map(int, y_val)), cfg=cfg, time_crop=args.time_crop,
                             specaugment=False, enable_shift=False, n_mels_override=args.n_mels)
    ...
    # ---- Warmup (freeze CNN) + freeze BN stats by putting backbone in eval
